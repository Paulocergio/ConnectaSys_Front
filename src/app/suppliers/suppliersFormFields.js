...
const InputField = memo(({ label, name, icon, value, onChange, onInput, type, required, errorMessage, maxLength }) => {
  return (
    <div className="mb-4">
...
const SuppliersFormFields = memo(({ formData, onChange, onFill }) => {
  ...
  const handleDocInput = useCallback(debounce(async (value) => {
  ...
  return (
    <>
      <InputField
        ...
        maxLength={18}
      />
      <InputField label="Razão Social" name="company_name" icon={<User />} value={formData.company_name} onChange={onChange} required />
      <InputField label="Nome do Contato" name="contact_name" icon={<User />} value={formData.contact_name} onChange={onChange} />
      <InputField
        label="E-mail"
        name="email"
        icon={<Mail size={18} />}
      ...
      />
      <InputField label="Telefone" name="phone" icon={<Phone />} value={formData.phone} onChange={onChange} />
      <InputField label="Endereço" name="address" icon={<MapPin />} value={formData.address} onChange={onChange} />
